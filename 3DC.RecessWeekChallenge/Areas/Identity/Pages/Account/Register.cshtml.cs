...
namespace _3DC.RecessWeekChallenge.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (currentUser.Email.Equals("jason_chow@mymail.sutd.edu.sg"))
                    {
                        var roleResult = await _userManager.AddToRoleAsync(currentUser, "Administrator");

                        if (roleResult.Succeeded)
                        {
                            _logger.LogInformation("Added to admin role");
                        }
                    }
